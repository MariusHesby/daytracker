...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const loadEntriesForDateRange = useCallback(
    async (start: string, end: string) => {
      ...
      if (user) {
        const loadedEntries = await cloudDb.getEntriesFromSupabase(user.id, start, end);
    ...
    async (entry: Omit<LogEntry, "id" | "createdAt" | "updatedAt">) => {
      if (user) {
        ...
        if (typeof entry.value === "string" && entry.value.trim()) {
          await cloudDb.addOrUpdateSuggestionInSupabase(user.id, entry.activityTypeId, entry.value);
        }
      } else {
